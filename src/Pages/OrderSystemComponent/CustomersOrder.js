...
function CustomersOrder() {
   ...
   const resetQuantHandler = () => {
      ...
      const resetMerch = [];
      newMerchandises.forEach((element) => {
         const tempElement = { ...element };
         console.log(tempElement);

         tempElement.merchQuant = 0;
         resetMerch.push(tempElement);
      });

      setMerchandise([...resetMerch]);
   };
                  ...
                  <Card.Body>
                     <form className={formValidated ? 'was-validated' : ''} noValidate onSubmit={handleSubmit} ref={formRef}>
                        ...
                        <Form.Group className="mb-3 has-validation">
                           ...
                           <div className="invalid-feedback">Your address is required</div>
                        </Form.Group>
                        <Form.Group className="mb-3 has-validation">
                           <Row>
                              <Col>
                              ...
                              <Col>
                                 <Select
                                    className={`react-select ${formSelectErrors && formSelectErrors.customersReceivingMethod ? 'invalid' : ''}`}
                                    innerProps={{ className: 'react-select-inner' }}
                                    options={options}
                                    value={formValues.customersReceivingMethod}
                                    onChange={changeSelectValueHandler}
                                    name="customersReceivingMethod"
                                    placeholder={formValues.customersReceivingMethod === '' ? 'Choose Recieve Method' : formValues.customersReceivingMethod}
                                    required
                                    isSearchable={false}
                                 />
                                 {formSelectErrors && formSelectErrors.customersReceivingMethod && <div className="select-invalid-feedback">Receive method is required</div>}
                              </Col>
                           </Row>
                        </Form.Group>
